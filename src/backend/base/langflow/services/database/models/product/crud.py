...
async def get_products(session: AsyncSession) -> list[ProductRead]:
    ...
    stmt = (
        select(ProductTable).options(selectinload(ProductTable.prices))  # This eagerly loads the prices
    )
...
async def create_product(session: DbSession, product: ProductCreate) -> ProductTable:
...
async def update_product(session: AsyncSession, product_id: str, **update_data) -> ProductTable | None:
    """Update a product."""
    ...
    try:
        ...
        logger.error(f"Error updating product: {e!s}", exc_info=True)
        raise
