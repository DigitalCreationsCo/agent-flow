...
async def get_prices(session: AsyncSession, active: Optional[bool] = None) -> List[PriceRead]:
    """Get all prices."""
    ...
    return [
        PriceRead.model_validate({
            **price.__dict__,
            'stripe_metadata': {} if price.stripe_metadata is None else dict(price.stripe_metadata)
        })
        for price in prices
    ]

async def get_price(session: AsyncSession, price_id: str) -> Optional[PriceTable]:
    """Get a price by ID."""
