...
from langflow.services.stripe.config import StripeConfig, get_stripe_config
from langflow.services.database.models.product.crud import (
    create_product,
    update_product,
    delete_product
)
from langflow.services.database.models.price.crud import (
    create_price,
    update_price,
    delete_price
)
...
async def handle_product_event(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to handle product event: {str(e)}"
        )
...
async def handle_price_event(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to handle price event: {str(e)}"
        )
...
async def handle_subscription_event(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to handle subscription event: {str(e)}"
        )
...
async def create_checkout_session(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="Unable to access customer record."
            ) from err
...
async def create_portal_session(
    current_path: str,
    current_user: CurrentActiveUser = get_current_active_user,
    session = Depends(DbSession)
) -> str:
    ...
    try:
        ...
        try:
            ...
            if not portal.url:
                raise HTTPException(
                    status_code=400,
                    detail="Could not create billing portal"
                )
            return portal.url
        except Exception as err:
            raise HTTPException(
...
async def create_or_get_stripe_customer(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error creating/getting Stripe customer: {str(e)}"
        )